...
import { KeyRound, LogIn, HelpCircle, X, Wrench } from 'lucide-react'
import PracticeRequestModal from './PracticeRequestModal'

export default function LoginClient({ isMaintenance = false }: { isMaintenance?: boolean }) {
    ...
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: '1rem' }}>
      <div className="glass-panel animate-in" style={{ width: '100%', maxWidth: '420px', padding: '2.5rem' }}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          <div style={{ display: 'inline-flex', background: isMaintenance ? 'rgba(239, 68, 68, 0.2)' : 'rgba(59, 130, 246, 0.2)', padding: '1rem', borderRadius: '50%', marginBottom: '1rem' }}>
            {isMaintenance ? <Wrench size={32} color="#ef4444" /> : <KeyRound size={32} color="#3b82f6" />}
          </div>
          <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem', color: isMaintenance ? '#ef4444' : 'inherit' }}>
            {isMaintenance ? 'Ведутся технические работы' : 'Единая База ШСО'}
          </h1>
          <p style={{ fontSize: '0.9rem' }}>
            {isMaintenance ? 'Вход доступен только для Разработчика' : 'Войдите с помощью уникального кода'}
          </p>
        </div>
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <input type="text" placeholder={isMaintenance ? "Код разработчика..." : "Ваш уникальный код..."} value={code} onChange={e => setCode(e.target.value)} className="input-field" required />
          {error && <div style={{ color: 'var(--danger-color)', fontSize: '0.875rem', textAlign: 'center' }}>{error}</div>}
          <button type="submit" className="btn-primary" disabled={loading} style={{ opacity: loading ? 0.7 : 1, background: isMaintenance ? '#ef4444' : undefined }}>
